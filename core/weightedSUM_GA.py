...
class Weighted_Sum_GA:
    ...
    @classmethod
    def _spearman_rank_corr(cls, x: np.ndarray, y: np.ndarray) -> float:
        x = np.asarray(x, dtype=np.float64).reshape(-1)
        y = np.asarray(y, dtype=np.float64).reshape(-1)
        if x.size != y.size or x.size < 2:
            return float("nan")

        rx = cls._rankdata_average_ties(x)
        ry = cls._rankdata_average_ties(y)
        rx = rx - rx.mean()
        ry = ry - ry.mean()
    ...
    def _compute_history_spearman(self, best_candidate) -> float:
        fitness = self.params["fitness"]
        x_adv = best_candidate.generate_adv_image().unsqueeze(0)
        try:
            adv_saliency, _ = fitness.explain_method(
                fitness.model,
                x_adv,
                fitness.normalize,
                fitness.y_true,
            )
            clean = fitness.saliency_true[0].detach().float().cpu().numpy().reshape(-1)
            adv = adv_saliency[0].detach().float().cpu().numpy().reshape(-1)
            return self._spearman_rank_corr(clean, adv)
        except Exception:
            return float("nan")

    def _compute_weighted_fitness(self, margin_losses, saliency_losses, del_losses=None, ins_losses=None):
        if del_losses is not None and ins_losses is not None:
            w_margin = float(self.params.get("w_margin", 0.34))
            w_del = self.params.get("w_del", None)
            w_ins = self.params.get("w_ins", None)
            if w_del is None and w_ins is None:
                base = float(self.params.get("w_saliency", 0.66))
                w_del = base * 0.5
                w_ins = base * 0.5
            else:
                w_del = 0.0 if w_del is None else float(w_del)
                w_ins = 0.0 if w_ins is None else float(w_ins)
            return w_margin * margin_losses + float(w_del) * del_losses + float(w_ins) * ins_losses
        return self.params['w_margin'] * margin_losses + self.params['w_saliency'] * saliency_losses

    def _build_best_scores(
        self,
        margin_losses,
        saliency_losses,
        weighted_fitness,
        ...
        best_scores = {
            'margin_loss': margin_losses[best_candidate_id],
            'saliency_loss': saliency_losses[best_candidate_id],
            'weighted_fitness': weighted_fitness[best_candidate_id],
            'first_success_iteration': first_success_iteration,
        }
        if del_losses is not None and ins_losses is not None:
            best_scores['del_loss'] = del_losses[best_candidate_id]
            best_scores['ins_loss'] = ins_losses[best_candidate_id]
        if spearman is not None:
            best_scores['spearman'] = spearman
        return best_scores

    def _pick_final_candidate_idx(
        self,
        population,
        weighted_fitness,
        del_losses=None,
        ...
        if del_losses is not None and ins_losses is not None:
            kept_indices = [i for i, sol in enumerate(population.population) if not sol.is_adversarial]
            if kept_indices:
                faith_scores = (del_losses + ins_losses) * 0.5
                return min(
                    kept_indices,
                    key=lambda idx: float(faith_scores[idx].detach().cpu().item()),
                )
        return int(torch.argmin(weighted_fitness).item())
    ...
    def attack(self):
        ...
        for it in tqdm(range(1, self.params["iterations"])):            
            ...
            off_margin_losses, off_saliency_losses, off_del_losses, off_ins_losses, off_logits = offpsrings.evaluate()  # calcuate fitenss
            ...
            if 'del_loss' in best_scores and 'ins_loss' in best_scores:
                print(
                    f"Iteration {it}: best weighted fitness = {best_scores['weighted_fitness']:.4f}, "
                    f"margin loss = {best_scores['margin_loss']:.4f}, del loss = {best_scores['del_loss']:.4f}, "
                    f"ins loss = {best_scores['ins_loss']:.4f}, spearman = {best_scores['spearman']:.4f}"
                )
            else:
                print(
                    f"Iteration {it}: best weighted fitness = {best_scores['weighted_fitness']:.4f}, "
                    f"margin loss = {best_scores['margin_loss']:.4f}, saliency loss = {best_scores['saliency_loss']:.4f}, "
                    f"spearman = {best_scores['spearman']:.4f}"
                )
