...
# Example:
#   W_MARGIN=0.7 W_DEL=0.2 W_INS=0.1 sbatch resnet18_NSGAII.sh
...
# Example override:
#   FITNESSES="ce_margin_loss_saliency" sbatch resnet18_NSGAII.sh
#   FITNESSES="margin_loss_causal_faithfull ce_margin_loss_saliency" sbatch resnet18_NSGAII.sh
    ...
    for STRATEGY in uniform; do
        for EPS in $EPSILONS; do
            for FITNESS in $FITNESSES; do
                echo "[RUN] model=$MODEL_NAME strategy=$STRATEGY fitness=$FITNESS eps=$EPS w_margin=$W_MARGIN w_del=$W_DEL w_ins=$W_INS explain_method=$EXPLAIN_METHOD num_sample=$NUM_SAMPLE output_root=$OUTPUT_ROOT algorithm=nsgaii"
                python run_batch.py \
